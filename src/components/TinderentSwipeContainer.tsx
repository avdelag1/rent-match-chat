...
export function TinderentSwipeContainer({ onListingTap, onInsights, onMessageClick, locationFilter, filters }: TinderentSwipeContainerProps) {
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center">
      ...
      {/* Bottom Action Bar - 3 Button Blurred Layout with safe-area inset */}
      <motion.div
        className="fixed bottom-0 left-0 right-0 z-20 backdrop-blur-xl bg-white/80 border-t border-white/20 shadow-2xl"
        style={{ paddingBottom: 'max(1rem, env(safe-area-inset-bottom))' }}
        initial={{ y: 100, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.2 }}
      >
        <div className="flex justify-center gap-6 items-center px-6 py-4">
          {/* Dislike Button */}
          <motion.div
          ...
          >
            <Button
              size="lg"
              variant="ghost"
              className="w-16 h-16 rounded-full bg-gradient-to-br from-rose-500 to-red-500 hover:from-rose-600 hover:to-red-600 text-white transition-all duration-300 shadow-xl p-0 border-0"
          ...
          >
            <Button
              ...
              className={`w-14 h-14 rounded-full transition-all duration-300 shadow-lg p-0 ${
                canUndo 
                  ? 'bg-white border-2 border-gray-400 text-gray-700 hover:bg-gray-50 hover:shadow-xl' 
                  : 'bg-gray-200 border-2 border-gray-300 text-gray-400 cursor-not-allowed opacity-50'
              }`}
          ...
          >
            <Button
              size="lg"
              variant="ghost"
              className="w-16 h-16 rounded-full bg-gradient-to-br from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white transition-all duration-300 shadow-xl p-0 border-0"
