...
export default function OwnerYachtClientDiscovery() {
  ...
  const clientFilters: ClientFilters | undefined = useMemo(() => {
    ...
    // Yacht types (category-specific)
    if (filters.yacht_types && filters.yacht_types.length > 0) {
      mapped.yachtTypes = filters.yacht_types;
    }
  ...
  const { data: clients = [], refetch } = useSmartClientMatching(user?.id, 'yacht', 0, 10, false, clientFilters);
        ...
        <div className="border-b bg-card">
          <div className="container mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold">Yacht Clients</h1>
        ...
        <div className="lg:hidden fixed z-50" style={{ bottom: 'calc(1.5rem + env(safe-area-inset-bottom, 0px))', right: '1.5rem' }}>
          <Sheet>
            ...
            <SheetContent side="bottom" className="h-[70vh] max-h-[600px]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
              <SheetHeader>
                <SheetTitle>Filter Yacht Clients</SheetTitle>
              </SheetHeader>
