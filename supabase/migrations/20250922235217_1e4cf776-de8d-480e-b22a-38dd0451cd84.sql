...
ALTER TABLE listings_browse ENABLE ROW LEVEL SECURITY;

-- 2. Create missing policies for tables that have RLS enabled but no policies
-- For geography_columns and geometry_columns (these are PostGIS system tables)
CREATE POLICY "Allow public read access to geometry_columns" ON geometry_columns FOR SELECT USING (true);
CREATE POLICY "Allow public read access to geography_columns" ON geography_columns FOR SELECT USING (true);

-- 3. Create reviews table for rating system
...
-- 4. Create message attachments table for enhanced messaging
...
-- 5. Create storage buckets for attachments if they don't exist
...
-- 6. Create storage policies for message attachments
...
-- 7. Create storage policies for review images
