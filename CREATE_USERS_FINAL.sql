-- ================================================================
-- CREATE TEST USERS - Clean version (no updated_at in user_roles)
-- ================================================================
...
BEGIN
    ...
    INSERT INTO public.message_activations (user_id, activation_type, total_activations, remaining_activations, used_activations, expires_at, notes, created_at, updated_at)
    VALUES (user1_id, 'referral_bonus', 5, 5, 0, NOW() + INTERVAL '90 days', 'Test user - 5 free messages', NOW(), NOW());
    ...
    INSERT INTO public.message_activations (user_id, activation_type, total_activations, remaining_activations, used_activations, expires_at, notes, created_at, updated_at)
    VALUES (user2_id, 'referral_bonus', 5, 5, 0, NOW() + INTERVAL '90 days', 'Test user - 5 free messages', NOW(), NOW());

    RAISE NOTICE '✅ Test users created!';
    RAISE NOTICE 'User 1: test1@zwipes.com / TestPass123! (CLIENT)';
    RAISE NOTICE 'User 2: test2@zwipes.com / TestPass123! (OWNER)';
END $$;

-- Recreate working trigger for future signups
...
BEGIN
    ...
    INSERT INTO public.message_activations (user_id, activation_type, total_activations, remaining_activations, used_activations, expires_at, notes, created_at, updated_at)
    VALUES (NEW.id, 'referral_bonus', 1, 1, 0, NOW() + INTERVAL '90 days', 'Welcome bonus', NOW(), NOW())
...
-- Verify
SELECT '✅ Created' as status, email, role FROM public.profiles WHERE email LIKE 'test%@zwipes.com' ORDER BY email;
